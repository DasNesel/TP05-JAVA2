...
The first step is to download the starting project [`java2_pw05_movies.zip`](./java2_pw05_movies.zip) and to extract it inside your *IDE* workspace.
...
The Java project contains two DAO *classes* in the `fr.isen.java2.db.daos package`: `FilmDao` and `GenreDao`. (One DAO for each entity and for each `SQL` table)
...
The FilmDao interface is similar to the GenreDao one. Its implementation is a little more difficult to write because the Film entity is little more complex than the Genre entity.

Once again, here are the SQL queries to use:

*method*|SQL Query
---|---
listFilms|`SELECT * FROM film JOIN genre ON film.genre_id = genre.idgenre`
listFilmsByGenre|`SELECT * FROM film JOIN genre ON film.genre_id = genre.idgenre WHERE genre.name = 'Comedy'`
addFilm|`INSERT INTO film(title,release_date,genre_id,duration,director,summary) VALUES(?,?,?,?,?,?)`

You will see that the `addFilm` *method* has a `Film` *object* as a *parameter* and as a *return* type. The *parameter* will have all the information needed to create the film in the database but no `id`! The *object* returned by the method must have the same information with the `id` that has been generated by the database.

Use the `FilmDaoTestCase` *class* to test your methods. This time, you have to write the tests yourself.

### Bonus stage

`DataSourceFactory` uses an explicit dependency to the SQLite driver. What if we want to use Postgre or MySQL? As for now we have to change our code and recompile. Not very clever, isn't it?
...
- The code should implement the requirement sof the PW.
...
!!! danger "Reminder"
    You have until **February, the 7th at 23:59:59.999** to send me your work.

You have two ways to send your work:

- Send me the link to a GitHub or Gitlab project (last commit **before** the deadline)
- Send me a zip file (max 10 mo) by email (change the extension, or send a 7zip file, to avoid having your file filtered by anti-spam software...);

Your code / email have to clearly state who you are.

It is your responsibility to ensure that I received the file. If I received your code I will get back to you individually to confirm that I received your work no later than March, the 8th at 23:59.
